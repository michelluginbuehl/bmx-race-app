...
export default function App() {
  ...
  const getOverallPreviewRows = (): Array<{ race: RaceName; count: number; status: string }> =>
    RACES.map((race) => {
      ...
      return { race, count, status: getRaceStatus(race) };
    });
  ...
  const downloadExcelTemplate = () => {
    const rows = [
      { Name: "Max Muster", Plate: "23", Verein: "BMX Club", Jahrgang: "2014", Geschlecht: "B", Cruiser: "", Race1: "x", Race2: "x", Race3: "x", Race4: "x" },
      { Name: "Lina Beispiel", Plate: "41", Verein: "BMX Club", Jahrgang: "2015", Geschlecht: "G", Cruiser: "x", Race1: "x", Race2: "", Race3: "x", Race4: "" },
    ];
    const ws = XLSX.utils.json_to_sheet(rows, { header: ["Name", "Plate", "Verein", "Jahrgang", "Geschlecht", "Cruiser", "Race1", "Race2", "Race3", "Race4"] });
  ...
  const findRiderStartInfo = (rider: any) => {
    ...
    const raceInfos = RACES.map((race) => {
  ...
  const getChangeLogFilterOptions = () => ["Alle", "Teilnehmer", "Race 1", "Race 2", "Race 3", "Race 4", "Gesamtwertung", "PDF", "Backup"];
...
${summary}`)) return;
  ...
  const exportBackup = async () => {
    try {
      ...
      const backupEntry = { fileName, iso: new Date().toISOString(), riderCount: ridersBackup.length };
      setBackupHistory((prev) => [backupEntry, ...prev].slice(0, 12));
      setBackupMessage(`Backup erstellt: ${fileName}`);
      addChangeLog(`Backup exportiert: ${fileName}`);
    } catch (error: any) {
      setBackupMessage("");
      alert(
  ...
  if (viewMode === "dashboard") {
    ...
    return (
      ...
      >
        <h1 style={{ color: colors.title, marginTop: 0, letterSpacing: "-0.02em" }}>
          🚴 BMX Racing Software
        </h1>
        <div style={{ color: colors.muted, fontWeight: 800, marginTop: -8, marginBottom: 18 }}>
          {APP_VERSION} · Rennverwaltung, Resultate und PDFs
        </div>
        ...
        <div style={{ ...basePanelStyle, marginBottom: 20 }}>
          <h2 style={{ marginTop: 0, color: colors.title }}>Fahrer-Suche</h2>
          <input
            value={globalSearch}
            onChange={(e) => setGlobalSearch(e.target.value)}
            placeholder="Name, Startnummer, Verein oder Kategorie suchen..."
            style={{ width: "100%", boxSizing: "border-box", padding: "14px 16px", borderRadius: 10, border: "1px solid #cfd8e3", fontSize: 18, fontWeight: 700 }}
          />
          {globalSearch.trim() && (
            <div style={{ marginTop: 12, display: "grid", gap: 8 }}>
              {globalSearchResults.length === 0 ? (
                <div style={{ color: colors.muted }}>Keine Fahrer gefunden.</div>
              ) : (
                globalSearchResults.map((r: any) => (
                  <button key={r.id} onClick={() => openRiderInfo(r)} style={{ ...secondaryButtonStyle, textAlign: "left", display: "grid", gridTemplateColumns: "90px 1fr 130px 220px", gap: 10, alignItems: "center" }}>
                    <strong>#{r.plate}</strong>
                    <span>{r.name}</span>
                    <span>{getRiderMetaLabel(r)}</span>
                    <span style={getCategoryBadgeStyle(r.category)}>{r.category}</span>
                  </button>
                ))
              )}
            </div>
        ...
        <div style={{ ...basePanelStyle, marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
            ...
            <div><strong>Letztes Backup</strong><br />{getLastBackup() ? `${new Date(getLastBackup().iso).toLocaleString("de-CH")} · ${getLastBackup().fileName}` : "Noch kein Backup"}</div>
            <div style={{ color: getBackupAgeMinutes() !== null && getBackupAgeMinutes()! > 60 ? colors.redBtn : colors.muted, fontWeight: 800 }}>
              {getBackupAgeMinutes() === null ? "Backup empfohlen" : getBackupAgeMinutes()! > 60 ? `Seit ${getBackupAgeMinutes()} Minuten kein Backup` : "Backup aktuell"}
            </div>
          </div>
        </div>

        <div style={{ ...basePanelStyle, marginBottom: 20 }}>
          <h2 style={{ marginTop: 0, color: colors.title }}>Race-Status</h2>
          <div
            style={{
              display: "grid",
          ...
          >
            {RACES.map((race) => (
              ...
              >
                <div
                  style={{ fontSize: 20, fontWeight: 900, marginBottom: 10 }}
                >
                  {race}
                </div>
                <span style={getStatusBadgeStyle(getRaceStatus(race))}>
                  {getRaceStatus(race)}
                </span>
                <div
                  style={{
                ...
                >
                  Teilnehmer: {getRaceParticipantCount(race)}
                </div>
              </button>
            ))}
          </div>
        ...
        <div style={{ ...basePanelStyle, marginBottom: 20 }}>
          <h2 style={{ marginTop: 0, color: colors.title }}>Schnellzugriff</h2>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
            ...
            <button onClick={downloadExcelTemplate} style={secondaryButtonStyle}>
              ...
              <div style={{ marginTop: 8, display: "grid", gap: 6 }}>
                {backupHistory.slice(0, 5).map((entry: any, index: number) => (
                  <div key={`${entry.fileName}-${index}`} style={{ color: colors.muted }}>
                    {new Date(entry.iso).toLocaleString("de-CH")} · {entry.fileName} · {entry.riderCount ?? "?"} Teilnehmer
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ ...basePanelStyle }}>
          ...
          <div style={{ marginTop: 14, color: colors.muted, fontWeight: 800 }}>
            BMX Racing Software {APP_VERSION}
          </div>
        </div>
      </div>
    );
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ ...basePanelStyle, marginBottom: 20 }}>
          ...
          >
            ...
            <button style={activeRaceButtonStyle}>{selectedRace}</button>
          </div>
        </div>
            ...
            {RACES.map((race) => (
              <div
                ...
                style={{
                  padding: "10px 12px",
                  borderRadius: 10,
                  background: race === selectedRace ? "#e8f1ff" : "#f8fafc",
        ...
        <div style={stickyButtonBarStyle}>
          ...
          >
            ...
            <button
              onClick={resetHeats}
              disabled={raceClosed}
              style={raceClosed ? disabledButtonStyle : secondaryButtonStyle}
            >
              Reset
            </button>
            <button
              onClick={toggleRaceClosed}
              style={raceClosed ? dangerButtonStyle : secondaryButtonStyle}
            >
            ...
            <button onClick={() => setViewMode("dashboard")} style={homeButtonStyle}>
              Home
            </button>
            <button
