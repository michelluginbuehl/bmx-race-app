...
export default function App() {
  ...
  const deleteAllRiders = async () => {
    if (!window.confirm("Alle Teilnehmer wirklich löschen? Diese Aktion kann nur mit einem Backup rückgängig gemacht werden.")) return;
    await exportBackup();
  ...
  const deleteAllRaceAssignments = async () => {
    if (!window.confirm(`${selectedRace}: Race-Zuordnungen wirklich löschen?`)) return;
  ...
  const getDuplicatePlateGroups = (items: any[]) => {
    const plateMap = new Map<string, { category: string; plate: string; names: string[]; ids: string[] }>();
    ...
    items.forEach((r: any) => {
      ...
      const entry = plateMap.get(key) || { category, plate, names: [], ids: [] };
      entry.names.push(String(r.name || "Ohne Name").trim());
      entry.ids.push(String(r.id || ""));
      plateMap.set(key, entry);
    });

    return Array.from(plateMap.values()).filter((entry) => entry.names.length > 1);
  };
  ...
  const getRiderValidationIssues = (items: any[]) => {
    ...
    const duplicates = getDuplicatePlateGroups(items).map(
      (entry) => `${entry.category} - #${entry.plate}: ${entry.names.join(", ")}`,
    );
  ...
  const validateSelectedRaceBeforeBuild = () => {
    ...
    if (issues.missing.length === 0 && issues.duplicates.length === 0) return true;

    alert(
      [
        "Bitte Teilnehmerdaten vor dem Erstellen der Läufe korrigieren.",
        ...
        issues.missing.length
          ...
          : "",
        issues.duplicates.length + issues.missing.length > 12
  ...
  const getRaceStatus = (race: RaceName) => {
    const heatData =
      race === selectedRace ? heats : JSON.parse(localStorage.getItem(getRaceStorageKey(race, "heats")) || "{}");
    const finalData =
      race === selectedRace ? finals : JSON.parse(localStorage.getItem(getRaceStorageKey(race, "finals")) || "{}");
    const finalResultData =
      race === selectedRace
        ? finalResults
        : JSON.parse(localStorage.getItem(getRaceStorageKey(race, "final_results")) || "{}");
    const closed =
      race === selectedRace
        ? raceClosed
    ...
    if (Object.values(finalResultData || {}).some((x: any) => Array.isArray(x) && x.length > 0))
  ...
  const findRiderStartInfo = (rider: any) => {
    ...
    Object.keys(heats || {}).forEach((cat) => {
      (heats[cat] || []).forEach((runGroups: any[], runIndex: number) => {
    ...
    Object.keys(finals || {}).forEach((cat) => {
      Object.keys(finals[cat] || {}).forEach((roundName) => {
        const found = (finals[cat][roundName] || []).find(
          (x: any) => String(x.id ?? x.riderId) === riderId,
        );
        if (found) {
          finalsInfo.push({
            roundName,
            startPos: found.startPos || "-",
            category: getFinalCategoryLabel(cat),
          });
        }
      });
    });

    return { rider, heatsInfo, finalsInfo };
  };

  const openRiderInfo = (rider: any) => setSelectedRiderInfo(findRiderStartInfo(rider));
  ...
  const createHeats = () => {
    ...
    if (Object.keys(heats || {}).length > 0 && !window.confirm("Vorläufe neu erstellen? Bestehende Vorläufe/Resultate werden überschrieben.")) return;
  ...
  const resetHeats = () => {
    if (!window.confirm(`${selectedRace} wirklich zurücksetzen? Vorläufe, Finals und Resultate dieses Race werden gelöscht.`)) return;
    setRaceClosed(false);
    setHeats({});
    setResults({});
    setFinals({});
  ...
  const saveHeatResult = (
    ...
    if (raceClosed) {
      alert("Dieses Race ist abgeschlossen. Resultate können erst nach dem Wiederöffnen geändert werden.");
      return;
    }
  ...
  const createFinals = () => {
    ...
    if (Object.keys(finals || {}).length > 0 && !window.confirm("Finals neu erstellen? Bestehende Finalresultate werden gelöscht.")) return;
    const all: any = {};
  ...
  const addPdfHeader = (
    ...
    if (showEventInfo) {
      ...
      doc.setFontSize(9);
      doc.setTextColor(31, 42, 55);
      doc.text(`Serie: ${eventSeries || "-"}`, eventInfoX + 3, 17);
      doc.text(`Ort: ${eventLocation || "-"}`, eventInfoX + 3, 24);
      doc.text(`Datum: ${eventDate || "-"}`, eventInfoX + 3, 31);
    }
  ...
  const addPdfPageNumbers = (doc: jsPDF) => {
    ...
    for (let page = 1; page <= totalPages; page += 1) {
      ...
      doc.text(`Seite ${page} / ${totalPages}`, pageWidth - 32, pageHeightValue - 7);
    }
  };
  ...
  const selectAllForRace = async (race: RaceName, category?: string) => {
    ...
    if (!window.confirm(`${race}: ${category ? "Kategorie " + category : "alle Teilnehmer"} wirklich ${nextValue ? "auswählen" : "abwählen"}?`)) return;
  ...
  const buildRacePointsMap = (race: RaceName) => {
    ...
    Object.keys(byOriginalCategory).forEach((category) => {
      ...
      orderedRanking.forEach((r: any, index: number) => {
  ...
  const createOverallRanking = () => {
    if (!window.confirm("Gesamtwertung jetzt aus den gespeicherten Finalresultaten erstellen/aktualisieren?")) return;
  ...
  const toggleRaceClosed = () => {
    if (raceClosed) {
      if (!window.confirm(`${selectedRace} wieder öffnen? Danach können Resultate wieder geändert werden.`)) return;
      setRaceClosed(false);
      return;
    }

    if (!window.confirm(`${selectedRace} abschliessen und sperren? Vorläufe, Finals und Resultate sind danach gegen versehentliche Änderungen geschützt.`)) return;
    setRaceClosed(true);
  };
      ...
      >
        ...
        <div style={{ ...basePanelStyle, marginBottom: 20 }}>
          ...
          >
            ...
            <button onClick={createOverallRanking} style={mainButtonStyle}>
      ...
      >
        ...
        <div style={{ ...basePanelStyle }}>
          ...
          {(() => {
            ...
            if (!issues.missing.length && !issues.duplicates.length) return null;
            return (
              <div style={{ ...basePanelStyle, borderColor: "#f0b429", background: "#fff8e6", marginBottom: 14 }}>
                <strong>Prüfung Teilnehmerdaten</strong>
                {issues.duplicates.length > 0 && (
                  <div style={{ marginTop: 8 }}>
                    Doppelte Startnummern pro Kategorie: {issues.duplicates.slice(0, 6).join(" | ")}
                    <div style={{ marginTop: 4 }}>Betroffene Fahrer sind unten rot markiert.</div>
                  </div>
                )}
                {issues.missing.length > 0 && (
                  <div style={{ marginTop: 8 }}>
                    Fehlende Pflichtfelder: {issues.missing.slice(0, 6).join(" | ")}
                  </div>
                )}
              </div>
            );
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ ...basePanelStyle, marginBottom: 20 }}>
          <strong style={{ color: colors.title }}>Race-Status</strong>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginTop: 10 }}>
            {RACES.map((race) => (
              <div key={race} style={{ padding: "10px 12px", borderRadius: 10, background: race === selectedRace ? "#e8f1ff" : "#f8fafc", border: "1px solid #d8e0e6", fontWeight: 800 }}>
                {race}: {getRaceStatus(race)}
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={stickyButtonBarStyle}>
          ...
          >
            <button
              ...
              style={heatsCreated || raceClosed ? disabledButtonStyle : mainButtonStyle}
              title={
                heatsCreated
            ...
            <button onClick={resetHeats} disabled={raceClosed} style={raceClosed ? disabledButtonStyle : secondaryButtonStyle}>
              Reset
            </button>
            <button onClick={toggleRaceClosed} style={raceClosed ? dangerButtonStyle : secondaryButtonStyle}>
            ...
            <button onClick={scrollHome} style={secondaryButtonStyle}>
              Home
            </button>
            <button
        ...
        {raceClosed && (
          <div style={{ ...basePanelStyle, marginBottom: 20, borderColor: colors.redBtn, background: "#fff1f1", fontWeight: 800 }}>
          ...
          >
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", gap: 20, alignItems: "flex-start" }}>
                <div>
                  <h2 style={{ marginTop: 0, color: colors.title }}>
                    #{selectedRiderInfo.rider.plate} {selectedRiderInfo.rider.name}
                  </h2>
                  <div style={{ color: colors.muted, fontWeight: 700 }}>
                    {selectedRiderInfo.rider.category} · {getRiderMetaLabel(selectedRiderInfo.rider)} · {selectedRiderInfo.rider.club || "-"}
                  </div>
                </div>
                <button onClick={() => setSelectedRiderInfo(null)} style={secondaryButtonStyle}>
                  Schliessen
                </button>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, marginTop: 18 }}>
                <div style={basePanelStyle}>
                  <h3 style={{ marginTop: 0, color: colors.title }}>Vorläufe</h3>
                  {selectedRiderInfo.heatsInfo.length === 0 ? (
                    <div style={{ color: colors.muted }}>Noch keine Vorläufe erstellt.</div>
                  ) : (
                    selectedRiderInfo.heatsInfo.map((item: any) => (
                      <div key={`${item.run}-${item.heat}`} style={{ padding: "10px 0", borderTop: "1px solid #eef2f6" }}>
                        <strong>Vorlauf {item.run}</strong><br />
                ...
                <div style={basePanelStyle}>
                  ...
                  {selectedRiderInfo.finalsInfo.length === 0 ? (
                    <div style={{ color: colors.muted }}>Noch keine Finals erstellt.</div>
                  ) : (
                    selectedRiderInfo.finalsInfo.map((item: any) => (
                      <div key={`${item.roundName}-${item.startPos}`} style={{ padding: "10px 0", borderTop: "1px solid #eef2f6" }}>
                        <strong>{item.roundName}</strong><br />
