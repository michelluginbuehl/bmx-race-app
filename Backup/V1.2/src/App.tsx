...
export default function App() {
  ...
  const addChangeLog = (text: string) => {
    ...
    setChangeLog((prev) => [`${time} ${text}`, ...prev].slice(0, 30));
  };
  ...
  const buildFinalCategoryRanking = (cat: string, useManualOrder = true) => {
    ...
    return sortRaceResultRows(ordered).map((item, index) => ({ ...item, rank: index + 1 }));
  };
  ...
  const buildRacePointsMap = (race: RaceName) => {
    ...
    Object.keys(byOriginalCategory).forEach((category) => {
      ...
      sortRaceResultRows(orderedRanking).forEach((r: any, index: number) => {
  ...
  const createOverallRanking = () => {
    if (
      !window.confirm(
        "Gesamtwertung jetzt aus den gespeicherten Finalresultaten erstellen/aktualisieren?",
      )
    )
      return;
    ...
    setOverallManualOrder({});
    addChangeLog("Gesamtwertung erstellt");
    setViewMode("overall");
  };
        ...
        <div style={{ ...basePanelStyle }}>
          ...
          {changeLog.length === 0 ? (
            ...
            changeLog.slice(0, 12).map((entry) => (
              <div
                key={entry}
                style={{ padding: "7px 0", borderTop: "1px solid #eef2f6" }}
              >
                {entry}
              </div>
            ))
