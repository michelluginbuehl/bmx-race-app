    ...
    try {
      ...
      rows.forEach((row, index) => {
        ...
        validRows.push({
          ...
          race1: getValue(row, ["Race 1", "Race1", "R1"]) === "" ? true : parseBoolean(getValue(row, ["Race 1", "Race1", "R1"])),
          race2: getValue(row, ["Race 2", "Race2", "R2"]) === "" ? true : parseBoolean(getValue(row, ["Race 2", "Race2", "R2"])),
          race3: getValue(row, ["Race 3", "Race3", "R3"]) === "" ? true : parseBoolean(getValue(row, ["Race 3", "Race3", "R3"])),
          race4: getValue(row, ["Race 4", "Race4", "R4"]) === "" ? true : parseBoolean(getValue(row, ["Race 4", "Race4", "R4"])),
        });
      });
      ...
      const preview = [
        `Excel-Import Vorschau: ${file.name}`,
        `Gefundene Zeilen: ${rows.length}`,
        `Gültige Teilnehmer: ${validRows.length}`,
        `Cruiser: ${validRows.filter((r) => r.cruiser).length}`,
        `Übersprungene Zeilen: ${skippedRows.length}`,
        duplicateRows.length ? `Doppelte Nummern in Datei: ${duplicateRows.slice(0, 6).join(" | ")}` : "Doppelte Nummern in Datei: keine",
        skippedRows.length ? `Fehler: ${skippedRows.slice(0, 6).join(" | ")}` : "",
        "",
        "Import bestätigen?",
      ].filter(Boolean).join("\n");
      ...
      if (validRows.length > 0) {
        await db.table("riders").bulkAdd(validRows);
      }

      setImportMessage(
        `${validRows.length} Teilnehmer importiert${skippedRows.length ? `, ${skippedRows.length} Zeilen übersprungen` : ""}.`,
      );
      await onChange();
    } catch (error: any) {
      alert(
